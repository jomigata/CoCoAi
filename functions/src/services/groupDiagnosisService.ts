...
// 그룹 진단 질문 인터페이스
interface GroupDiagnosisQuestion {
  id: string;
  category: 'communication' | 'trust' | 'conflict' | 'support' | 'goals' | 'intimacy' | 'leadership' | 'cohesion';
  question: string;
  type: 'scale' | 'choice' | 'ranking' | 'multiple-choice';
  options?: string[];
  scaleRange?: { min: number; max: number; labels: string[] };
  required: boolean;
  groupTypes: string[]; // 적용 가능한 그룹 유형
  weight: number; // 채점 가중치
}

// 그룹 진단 응답 인터페이스
interface GroupDiagnosisResponse {
  userId: string;
  questionId: string;
  response: any;
  timestamp: Date;
...
// 그룹 진단 결과 인터페이스
interface GroupDiagnosisResult {
  groupId: string;
  groupType: string;
  memberCount: number;
  completedAt: Date;
  responses: GroupDiagnosisResponse[];
  analysis: {
...
export class GroupDiagnosisService {
  private database = db;

  /**
   * 그룹 유형별 맞춤형 질문 세트 생성
   */
  private getGroupTypeQuestions(): { [groupType: string]: GroupDiagnosisQuestion[] } {
    return {
      'family': [
        // 가족 특화 질문들
        {
   ...
   * 공통 질문들 (모든 그룹 유형에 적용)
   */
  private getCommonQuestions(): GroupDiagnosisQuestion[] {
    return [
      {
   ...
   * 그룹 유형별 맞춤형 질문 세트 반환
   */
  async getGroupDiagnosisQuestions(groupType: string): Promise<GroupDiagnosisQuestion[]> {
    ...
    const applicableCommonQuestions = commonQuestions.filter(q => 
      q.groupTypes.includes(groupType)
    );
   ...
   * 그룹 진단 응답 분석 및 결과 생성
   */
  async analyzeGroupDiagnosis(
    groupId: string,
    groupType: string,
    responses: GroupDiagnosisResponse[]
  ): Promise<GroupDiagnosisResult> {
    try {
      // 1. 기본 점수 계산
      ...
      // 6. AI 기반 심층 분석
      const aiInsights = await this.performAIGroupAnalysis(
        groupType,
        categoryScores,
        memberAnalysis,
        groupDynamics
      ...
      // 7. 맞춤형 추천 생성
      const recommendations = await this.generateGroupRecommendations(
        groupType,
        categoryScores,
        strengths,
        weaknesses
      ...
      const result: GroupDiagnosisResult = {
        groupId,
        groupType,
        ...
        groupDynamics,
        aiInsights,
        aiWarning: AIWarningService.generateContextualWarning('group_analysis', {
   ...
   * 카테고리별 점수 계산
   */
  private calculateCategoryScores(responses: GroupDiagnosisResponse[]): any {
   ...
   * 멤버별 기여도 분석
   */
  private async analyzeMemberContributions(
    groupId: string,
    responses: GroupDiagnosisResponse[]
  ): Promise<any> {
    // 멤버별 응답 그룹화
    const memberResponses: { [userId: string]: GroupDiagnosisResponse[] } = {};
   ...
   * 그룹 역학 분석
   */
  private analyzeGroupDynamics(responses: GroupDiagnosisResponse[], groupType: string): any {
    const categoryScores = this.calculateCategoryScores(responses);
   ...
   * AI 기반 그룹 분석
   */
  private async performAIGroupAnalysis(
    groupType: string,
    categoryScores: any,
    memberAnalysis: any,
    groupDynamics: any
...
`;
   ...
   * 그룹 진단 결과 저장
   */
  private async saveGroupDiagnosisResult(result: GroupDiagnosisResult): Promise<void> {
    await this.database.collection('group_diagnosis_results').doc(result.groupId).set({
      ...result,
   ...
   * 그룹 진단 결과 조회
   */
  async getGroupDiagnosisResult(groupId: string): Promise<GroupDiagnosisResult | null> {
    try {
  ...
  // 헬퍼 메서드들
  private getQuestionById(questionId: string): GroupDiagnosisQuestion | null {
    // 실제 구현에서는 질문 데이터베이스에서 조회
    return null;
  }

  private getChoiceScore(choice: string, question: GroupDiagnosisQuestion): number {
    // 선택형 답변의 점수 계산 로직
    return 3; // 기본값
  }

  private determineRoleInGroup(scores: any): string {
    if (scores.leadership >= 80) return '리더';
    if (scores.support >= 80) return '지지자';
    if (scores.communication >= 80) return '소통자';
    return '참여자';
  }

  private identifyContributionAreas(scores: any): string[] {
    const areas = [];
    if (scores.communication >= 70) areas.push('소통');
    if (scores.support >= 70) areas.push('지지');
    if (scores.goals >= 70) areas.push('목표 달성');
    return areas;
  }

  private identifyGrowthAreas(scores: any): string[] {
    const areas = [];
    if (scores.communication < 60) areas.push('소통 능력');
    if (scores.trust < 60) areas.push('신뢰 구축');
    if (scores.support < 60) areas.push('지지 제공');
    return areas;
  }

  private generatePersonalizedAdvice(scores: any, responses: GroupDiagnosisResponse[]): string[] {
    const advice = [];
  ...
  private determineLeadershipStyle(scores: any): string {
    if (scores.leadership >= 80) return '민주적 리더십';
    if (scores.leadership >= 60) return '협력적 리더십';
    return '자연스러운 리더십';
  }

  private determineCommunicationPattern(scores: any): string {
    if (scores.communication >= 80) return '개방적 소통';
    if (scores.communication >= 60) return '균형적 소통';
    return '신중한 소통';
  }

  private determineConflictResolutionStyle(scores: any): string {
    if (scores.conflict >= 80) return '협력적 해결';
    if (scores.conflict >= 60) return '타협적 해결';
    return '회피적 해결';
  }

  private determineSupportNetwork(scores: any): string {
    if (scores.support >= 80) return '강한 지지 네트워크';
    if (scores.support >= 60) return '적당한 지지 네트워크';
    return '약한 지지 네트워크';
  }
}
