...
export class PersonalProfilingService {
  ...
  async getPersonalizedQuestions(ageGroup: string): Promise<ProfilingQuestion[]> {
    ...
    // 공통 질문 추가
    const commonQuestions: ProfilingQuestion[] = [
      // 대인관계 패턴 (공통)
      {
   ...
   * 점수 계산
   */
  private calculateScores(responses: { [questionId: string]: any }, ageGroup: string): any {
    // 자아존중감 점수 계산
    const selfEsteemQuestions = Object.keys(responses).filter(key => 
      key.includes('self') || key.includes('worth') || key.includes('confidence')
    );
   ...
   * 스트레스 대처 방식 분류
   */
  private categorizeStressCoping(methods: string[], category: string): number {
    ...
    let categoryMethods: string[] = [];
    switch (category) {
      case 'active': categoryMethods = activeMethods; break;
      case 'passive': categoryMethods = passiveMethods; break;
      case 'social': categoryMethods = socialMethods; break;
      case 'individual': categoryMethods = individualMethods; break;
    }

    const matches = methods.filter(method => 
      categoryMethods.some(categoryMethod => method.includes(categoryMethod))
    ).length;
   ...
   * 마음 지도 생성
   */
  private generateMindMap(scores: any, responses: { [questionId: string]: any }): any {
    // 성격 유형 결정
    let personality = '';
    ...
    if (stressCoping.some((method: string) => method.includes('친구') || method.includes('가족'))) {
      emotionalPattern = '관계 중심형';
    } else if (stressCoping.some((method: string) => method.includes('혼자'))) {
      emotionalPattern = '내적 성찰형';
    } else if (stressCoping.some((method: string) => method.includes('문제 해결'))) {
    ...
    // 성장 영역 식별
    const growthAreas = [];
    if (scores.selfEsteem < 60) growthAreas.push('자아존중감 향상');
    if (scores.stressCoping.passive > 70) growthAreas.push('적극적 대처법 학습');
    if (scores.stressCoping.social < 30) growthAreas.push('사회적 지지망 구축');
    if (strengths.length < 5) growthAreas.push('강점 발견 및 활용');
...
`;
  ...
  private async saveProfilingResult(result: ProfilingResult): Promise<void> {
    await this.database.collection('profiling_results').doc(result.userId).set({
      ...result,
   ...
   * 프로파일링 결과 업데이트
   */
  async updateProfilingResult(userId: string, updates: Partial<ProfilingResult>): Promise<void> {
    try {
      await this.database.collection('profiling_results').doc(userId).update({
        ...updates,
        updatedAt: serverTimestamp()
      });
    } catch (error) {
