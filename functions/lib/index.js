 ...
 * 연령대별 맞춤형 질문 제공
 */
exports.getProfilingQuestions = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
        const { ageGroup } = data;
        const profilingService = new personalProfilingService_1.PersonalProfilingService();
 ...
 */
exports.analyzeProfilingResults = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
 ...
 * 🧠 개인 프로파일링 결과 조회 함수
 */
exports.getProfilingResult = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
 ...
 * 📧 그룹 초대 이메일 발송 함수
 */
exports.sendGroupInvitation = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
...
exports.processAIChat = functions.https.onCall(async (data, context) => {
    try {
        ...
        // 고도화된 AI 챗봇 서비스 사용
        const { AIChatService } = await Promise.resolve().then(() => __importStar(require('./services/aiChatService')));
 ...
 */
exports.analyzeMoodPatterns = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
...
exports.generateRecommendations = functions.https.onCall(async (data, context) => {
    try {
        ...
        switch (type) {
            case 'content':
                recommendations = await recommendationService.generateContentRecommendations(userId, data.category || 'article');
                break;
            case 'activity':
                recommendations = await recommendationService.generateActivityRecommendations(userId, data.timeAvailable || 30);
                break;
            default:
                recommendations = await recommendationService.generatePersonalizedRecommendations(userId, limit);
        }
        return {
            success: true,
            recommendations,
 ...
 * 📊 추천 피드백 저장 함수
 */
exports.saveRecommendationFeedback = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
 ...
 */
exports.getUserGarden = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
        const { userId } = data;
        const gardenService = new gardenService_1.GardenService();
 ...
 */
exports.performGardenAction = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
 ...
 */
exports.updateGardenFromGroupActivity = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
 ...
 */
exports.simulatePlantGrowth = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
...
exports.getUserStats = functions.https.onCall(async (data, context) => {
    try {
        ...
        const { userId } = data;
        const badgeService = new badgeService_1.BadgeService();
 ...
 * 🏆 뱃지 진행 상황 조회 함수
 */
exports.getUserBadgeProgress = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
 ...
 * 🏆 뱃지 획득 체크 및 업데이트 함수
 */
exports.checkAndUpdateBadges = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
 ...
 * 🏆 칭찬 릴레이 전송 함수
 */
exports.sendPraise = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
 ...
 * 🏆 칭찬 받은 목록 조회 함수
 */
exports.getReceivedPraises = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
        }
